...
# --- 4. Track 2: ONT-seq (Coverage + Reads) ---
# Ensure the BAM index (.bai) exists in the same folder, or Gviz might fail.
track_ont <- AlignmentsTrack(
  ONT_BAM,
  isPaired = FALSE,
  chromosome = chr_target,
  start = from_pos,
  ...
  # Use 'pack' to make the few reads visible
  stacking = "full",     
  
  # Optional: removing 'coverage' helps if the histogram looks blocky/ugly at low depth
  type = c("coverage", "pileup"), 
  
  showIndels = TRUE,
  fill.coverage = "#A0A0A0",  
  col.coverage = NA,
  col.mates = "#A0A0A0",
  col.reads = "#A0A0A0",
  background.title = "white",
...
# --- 5. Track 3: Gene Model (Helper Function) ---
...
# --- 6. Plotting ---
png("PF3D7_1476600_Three_Tracks.png", width = 10, height = 6, units = "in", res = 600)

tracks_to_plot <- list(track_rnaseq, track_ont, track_gene)
track_sizes <- c(1, 1, 0.1)
...
# --- 6. Plotting ---
png("refined_gene_figure.png", width = 12, height = 12, units = "in", res = 800)
