...
#SBATCH --mem=50G
#SBATCH -p epyc
#SBATCH --array=1-16   # <<< one task per marker; adjust to number of markers in JSON
...
DEFAULT_MARKER_JSON="${REPO_ROOT}/window1000_bin100/training_features_values/marker_files.local.json"
if [[ ! -f "${DEFAULT_MARKER_JSON}" ]]; then
  DEFAULT_MARKER_JSON="${REPO_ROOT}/window1000_bin100/training_features_values/marker_files.json"
fi
